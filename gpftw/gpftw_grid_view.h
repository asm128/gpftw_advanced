#include "gpftw_array_view.h"		// for gpftw::array_view<>
...
namespace gpftw 
{
	...
	public:
		...
		// Operators
		::gpftw::array_view<_tElement>			operator[]					(uint32_t row)														{ if(row >= Height) throw(::std::exception("Invalid row.")); return ::gpftw::array_view<_tElement>(&Data[row*Width], Width); }
		const ::gpftw::array_view<_tElement>	operator[]					(uint32_t row)												const	{ if(row >= Height) throw(::std::exception("Invalid row.")); return ::gpftw::array_view<_tElement>(&Data[row*Width], Width); }
