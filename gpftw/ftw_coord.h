...
#pragma pack(pop)
	...
	// ---- Collision
	template<typename _tElement>	static					bool					raySegmentIntersect		(::ftwlib::SCoord2<_tElement> r_d, ::ftwlib::SCoord2<_tElement> r_p, ::ftwlib::SCoord2<_tElement> s_d, ::ftwlib::SCoord2<_tElement> s_p)	{
		double																				t2						= (r_d.x * (s_p.y - r_p.y) + r_d.y * (r_p.x - s_p.x)) / (s_d.x * r_d.y - s_d.y * r_d.x);
		double																				t1						= (s_p.x + s_d.x * t2 - r_p.x) / r_d.x;	// Plug the value of T2 to get T1
