...
template <typename _tElem>
			int32_t																			renderParticle													(::ftwl::SASCIITarget& screenAscii, const ::ftwl::SParticle2Engine<_tElem>& particleEngine, int32_t particleIndex, uint8_t image, uint16_t color)				{
	...
	screenAscii.Characters	[(int32_t)screenPosition.y][(int32_t)screenPosition.x]				= image;
	screenAscii.Colors		[(int32_t)screenPosition.y][(int32_t)screenPosition.x]				= color | (screenAscii.Colors[(int32_t)screenPosition.y][(int32_t)screenPosition.x] & 0xF0);
	return 0;
}
...
static inline	::ftwl::error_t															printToScreen													(::ftwl::SASCIITarget & screenAscii, const ::ftwl::SCoord2<uint32_t>& offset, const ::ftwl::array_view<const char>& text, const ::ftwl::ASCII_COLOR color) {
	::memcpy(&screenAscii.Characters[offset.y][offset.x], text.begin(), text.size());
	for(uint32_t iCell = 0, cellCount = text.size(); iCell < cellCount; ++iCell)
		screenAscii.Colors[offset.y][offset.x + iCell]												= (::ftwl::ASCII_COLOR)(color | (screenAscii.Colors[offset.y][offset.x + iCell] & 0xF0)); 
	return 0;
}

template<typename... _argsToPrint>
static			::ftwl::error_t															printfToScreen													(::ftwl::SASCIITarget & screenAscii, const ::ftwl::SCoord2<uint32_t>& offset, const ::ftwl::ASCII_COLOR color, const char* format, _argsToPrint&&... argsToPrint) {
	char																							formatted[64]													= {};
...
//
			::ftwl::error_t																game::render													(SGame& gameInstance, ::ftwl::SASCIITarget& screenAscii)									{ 
	...
	for(uint32_t iCell = 0, cellCount = screenAscii.Colors.size(); iCell < cellCount; ++iCell)	// reset every cell's background color 
		screenAscii.Colors.begin()[iCell]														= ::ftwl::ASCII_COLOR_DARKBLUE << 4;
	{ // Render GUI
